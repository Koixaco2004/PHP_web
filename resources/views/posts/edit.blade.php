...
<div class="w-full">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
                        ...
                        <div class="flex space-x-3">
                            ...
                            @if(Auth::check() && (Auth::user()->id === $post->user_id || Auth::user()->role === 'admin'))
                                <button type="button" id="deletePostBtn" class="btn-secondary flex items-center text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300" onclick="showConfirmationModal('Xác nhận xóa', 'Bạn có chắc chắn muốn xóa bài viết này?', 'Xóa', () => { document.getElementById('deletePostForm').submit(); }, 'delete');">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                                    </svg>
                                    Xóa
    ...
    @if(Auth::check() && (Auth::user()->id === $post->user_id || Auth::user()->role === 'admin'))
        <form id="deletePostForm" action="{{ route('posts.destroy', $post) }}" method="POST" style="display: none;">
            @csrf
            @method('DELETE')
        </form>
    @endif
...
function confirmSubmit(event) {
    @if(Auth::check() && Auth::user()->role === 'admin' && $post->approval_status === 'pending')
        ...
        showConfirmationModal(
            ...
            'Bạn có chắc chắn muốn cập nhật và phê duyệt bài viết này?',
            'Xác nhận',
            function() {
