    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-secondary-200 dark:border-gray-700 p-6 animate-slide-up" style="animation-delay: 0.4s">
        <div class="flex items-center justify-between mb-4">
            ...
            <form method="GET" action="{{ route('admin.dashboard') }}" class="flex items-center">
                <label for="months" class="text-sm text-secondary-600 dark:text-gray-300 mr-2">Hiển thị:</label>
                <select name="months" id="months" class="bg-white dark:bg-gray-700 border border-secondary-300 dark:border-gray-600 rounded-md px-3 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" onchange="this.form.submit()">
                    <option value="3" {{ request('months', 6) == 3 ? 'selected' : '' }}>3 tháng</option>
                    <option value="6" {{ request('months', 6) == 6 ? 'selected' : '' }}>6 tháng</option>
                    <option value="12" {{ request('months', 6) == 12 ? 'selected' : '' }}>12 tháng</option>
                </select>
            </form>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Existing action cards animation
